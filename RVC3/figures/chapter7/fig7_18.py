...
def draw_puma(
    ...
    j1_pose = base_transform * SE3(0, 0, 0.623) * SE3.Rz(joint1_angle)
    ...
    j2_pose = part2_pose * SE3(0, -0.168733, 0) * SE3.Ry(joint2_angle-np.pi/2)
    ...
    j3_pose = part3_pose * SE3(0.425469, 0, 0) * SE3.Ry(joint3_angle+np.pi)
    ...
    j4_pose = part4_pose * SE3(0.0, 0.03806, -0.371487) * SE3.Rz(joint4_angle-np.pi/2)

    part5_pose = j4_pose * SE3(0.0, 0.0, 0.0)
    part = read_gltf(str(model_dir / 'Puma560_link5.glb'))
    a = plot_multicolour_mesh(plotter, part, part5_pose, name=f"{name}-link5")
    actors.append(a)

    j5_pose = part5_pose * SE3(0, 0, -0.05999) * \
              SE3.Rz(math.radians(90)) * SE3.Ry(math.radians(90) + joint5_angle)
...
def main():
    ...
    draw_puma(plotter, model_dir, *q[:5], base_transform=SE3.Rz(math.radians(90)))

    plotter.set_background('white')
    filename = rvcprint.outfile(format='png')
