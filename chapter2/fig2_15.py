...
add_plot(0, 0, SE3())
rib = frames.ribbon(phase=0, T= SE3(0.5, 0, 0) * SE3.Ry(-pi/2))
...
add_plot(0, 1, Rx)
rib = frames.ribbon(phase=-0.18, T= Rx * SE3(0, 0.45, 0) * SE3.Rx(pi/2))
plotter.add_mesh(rib, color=acolor)
print('--------')
...
## bottom row
add_plot(1, 0, SE3())
rib = frames.ribbon(phase=0, T= SE3(0, 0.5, 0) * SE3.Rx(pi/2))
plotter.add_mesh(rib, color='gray')
plotter.add_text('original pose', position='lower_edge', color='black')

add_plot(1, 1, Ry)
rib = frames.ribbon(phase=0.05, T= Ry * SE3(0.42, 0, 0) * SE3.Ry(-pi/2))
plotter.add_mesh(rib, color=acolor)
...
plotter.show(screenshot='fig2_15.png')