...
plt.plot(primaries[:, 0], primaries[:, 1], 'o')

green_cc = lambda2rg(500 * nm)
plt.plot(green_cc[0], green_cc[1], '*')

white_cc = tristim2cc([1, 1, 1])
plt.plot(white_cc[0], white_cc[1], 'H')
...
colorspace_convert([0, 0.5, 0] + [0.4, 0.4, 0.4], 'RGB', 'HSV')
...
xy = XYZ[:, :2] / np.sum(XYZ, axis=1)[..., np.newaxis]
ab = Lab[:, 1:]
